<div class="modal fade" id="modal_tambah">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<?= form_open('Master/Card/store', ['class' => 'form_create']) ?>
			<div class="modal-body">
				<div class="form-group">
					<label>Kode Seri</label>
					<input type="text" name="kode" class="form-control" placeholder="Contoh ID : 1305022802980001">
				...
				<div class="form-group ">
					<label>Room</label>
					<select class="form-control" name="room" id="level">
						<?php foreach ($room as $key ) { ?>
							<option value="<?= $key['id_room'] ?>"><?= $key['id_room']."-".$key['name_room'] ?></option>
						<?php } ?>
					</select>
					<span class="error room text-red"></span>
				</div>
			</div>
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.status == false) {
					...
					window.location.href = "<?= site_url('List_Card') ?>";
				}
			},
			complete: function() {
